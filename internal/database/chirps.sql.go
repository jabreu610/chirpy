...
const getFilteredChirps = `-- name: GetFilteredChirps :many
SELECT id, created_at, updated_at, body, user_id
FROM chirps
WHERE user_id = $1
`

func (q *Queries) GetFilteredChirps(ctx context.Context, userID uuid.UUID) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, getFilteredChirps, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
